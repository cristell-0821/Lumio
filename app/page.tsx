...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // TODO: descomentar cuando el proyecto esté listo para producción
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-10 p-6 rounded-2xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/20">
        <div className="flex items-start gap-3">
          ...
          {loadingGreeting ? (
            ...
            <p className="text-emerald-100 text-lg leading-relaxed">{greeting}</p>
          )}
        </div>
      </div>
      ...
      {/* Title */}
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-emerald-50">Dashboard</h2>
        <p className="text-emerald-600 mt-1">Resumen de tu actividad académica</p>
      </div>
        ...
        {cards.map(({ href, icon: Icon, label, value, description, gradient, border, iconColor }) => (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <ArrowRight className="text-emerald-700 group-hover:text-emerald-400 
                group-hover:translate-x-1 transition-all duration-200" size={18} />
            </div>
            <p className="text-4xl font-bold text-emerald-50 mb-1">{value}</p>
            <p className="text-sm text-emerald-600">
              <span className="text-emerald-400 font-medium">{label}</span> {description}
            </p>
          </Link>
        ))}
      </div>
